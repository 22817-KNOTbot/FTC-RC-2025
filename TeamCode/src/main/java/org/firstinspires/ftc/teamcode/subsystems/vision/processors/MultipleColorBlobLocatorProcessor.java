...
public class MultipleColorBlobLocatorProcessor extends ColorBlobLocatorProcessor implements VisionProcessor {
	...
	public MultipleColorBlobLocatorProcessor(List<List<Scalar>> colorRangeList,
			ContourMode contourMode, MorphOperationType morphOperationType, int erodeSize, int dilateSize,
			boolean drawContours, int blurSize, @ColorInt int boundingBoxColor, @ColorInt int circleFitColor,
			@ColorInt int roiColor, @ColorInt int contourColor) {
	...
	public void init(int width, int height, CameraCalibration calibration) {
		...
		// rect.x = (int) Range.scale(-1, -1, 1, 0, width);
		// rect.y = (int) (height - Range.scale(1, -1, 1, 0, height));
		// rect.width = (int) Range.scale(2, 0, 2, 0, width);
		// rect.height = (int) Range.scale(2, 0, 2, 0, height);

		// Adjust the window position to ensure it stays on the screen. push it back
	...
	public Object processFrame(Mat frame, long captureTimeNanos) {
		...
		for (List<Scalar> colorRange : colorRangeList) {
			...
			ArrayList<MatOfPoint> currentContours = new ArrayList<>();
			...
			switch (morphOperationType) {
				case OPENING:
					if (erodeElement != null) {
						Imgproc.erode(currentMask, currentMask, erodeElement);
					}

					if (dilateElement != null) {
						Imgproc.dilate(currentMask, currentMask, dilateElement);
					}
					break;
			...
			Imgproc.findContours(currentMask, currentContours, hierarchy, contourCode, Imgproc.CHAIN_APPROX_SIMPLE);
			hierarchy.release();

			contours.addAll(currentContours);
	...
	public void onDrawFrame(Canvas canvas, int onscreenWidth, int onscreenHeight, float scaleBmpPxToCanvasPx,
		...
		for (Blob blob : blobs) {
			if (drawContours) {
				...
				path.moveTo((float) (contourPts[0].x) * scaleBmpPxToCanvasPx,
						(float) (contourPts[0].y) * scaleBmpPxToCanvasPx);
				for (int i = 1; i < contourPts.length; i++) {
					path.lineTo((float) (contourPts[i].x) * scaleBmpPxToCanvasPx,
							(float) (contourPts[i].y) * scaleBmpPxToCanvasPx);
				}
				path.close();
