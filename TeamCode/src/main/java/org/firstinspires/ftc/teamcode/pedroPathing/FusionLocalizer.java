...
public class FusionLocalizer implements Localizer {
	...
	public void update() {
		...
		// Updates twist, note that the dead reckoning localizer returns world-frame
		// twist
		Pose twist = deadReckoning.getVelocity();
		twistHistory.put(now, twist.copy());
		currentVelocity = twist.copy();

		// Perform twist integration to propagate the fused position estimate based on
		// how the odometry thinks the robot has moved
		...
		if (poseHistory.size() > bufferSize)
			poseHistory.pollFirstEntry();
		if (twistHistory.size() > bufferSize)
			twistHistory.pollFirstEntry();
		if (covarianceHistory.size() > bufferSize)
			covarianceHistory.pollFirstEntry();
	}

	/**
	 * Consider the system xₖ₊₁ = xₖ + (f(xₖ, uₖ) + wₖ) * Δt.
	 * <p>
	 * wₖ is the noise in the system caused by sensor uncertainty, a zero-mean
	 * random vector with covariance Q.
	 * <p>
	 ...
	 * </pre>
	 * 
	 * Here F and G represent the State Transition Matrix and Control-to-State
	 * Matrix respectively.
	 * <p>
	 * The State Transition Matrix F is given by I + ∂f/∂x.
	 * We computed our twist integration using a first-order forward-Euler
	 * approximation.
	 ...
	 * 
	 * @param measuredPose the measured position by the camera, enter NaN to a
	 *                     specific axis if the camera couldn't measure that axis
	...
	/**
	 * Adds a vision measurement with a custom variance for this specific
	 * measurement
	 * 
	 * @param measuredPose        the measured position by the camera, enter NaN to
	 *                            a specific axis if the camera couldn't measure
	 *                            that axis
	 * @param timestamp           the timestamp of the measurement
	 * @param measurementVariance the variance for this specific measurement (x, y,
	 *                            heading), or null to use the default
	 */
	...
	// Performs linear interpolation inside the history map for the value at a given
	// timestamp
	private static Pose interpolate(long timestamp, NavigableMap<Long, Pose> history) {
		...
		if (lowerKey.equals(upperKey))
			return history.get(lowerKey).copy();
	...
	@Override
	public Pose getPose() {
		return currentPosition.withHeading(deadReckoning.getPose().getHeading());
	}
	...
	public void setPose(Pose setPose) {
		...
		if (poseHistory.lastEntry() != null)
			...
			poseHistory.put(System.nanoTime(), setPose.copy());
		else
	...
	public boolean isNAN() {
		return Double.isNaN(currentPosition.getX()) || Double.isNaN(currentPosition.getY())
