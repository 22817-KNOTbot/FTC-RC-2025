...
public class AutoPaths {
	public static class Red {
		...
		public static PathChain getExitLowShootingZone(Follower follower) {
			return follower.pathBuilder()
					.addPath(
							new BezierPoint(new Pose(96.000, 30.000)))
					.setConstantHeadingInterpolation(Math.toRadians(0))
					.build();
		}

		public static PathChain getExitUpShootingZone(Follower follower) {
			return follower.pathBuilder()
					.addPath(
							new BezierPoint(new Pose(96.000, 74.000)))
					.setConstantHeadingInterpolation(Math.toRadians(0))
					.build();
		}
	}
	...
	public static class Blue {
		public static PathChain getBottomApproach(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getBottomApproach(follower, startingPose), follower);
		}

		public static PathChain getMiddleApproach(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getMiddleApproach(follower, startingPose), follower);
		}

		public static PathChain getPushGate(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getPushGate(follower, startingPose), follower);
		}

		public static PathChain getTopApproach(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getTopApproach(follower, startingPose), follower);
		}

		public static PathChain getIntakePreparedSet(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getIntakePreparedSet(follower, startingPose), follower);
		}
		...
		public static PathChain getLowLaunch(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getLowLaunch(follower, startingPose), follower);
		}

		public static PathChain getUpLaunch(Follower follower, Pose startingPose) {
			return mirrorPathChain(Red.getUpLaunch(follower, startingPose), follower);
		}

		public static PathChain getExitLowShootingZone(Follower follower) {
			return mirrorPathChain(Red.getExitLowShootingZone(follower), follower);
		}

		public static PathChain getExitUpShootingZone(Follower follower) {
			return mirrorPathChain(Red.getExitUpShootingZone(follower), follower);
		}
	}
	...
	public static PathChain mirrorPathChain(PathChain pathChain, Follower follower) {
		...
		for (int i = 0; i < pathChain.size(); i++) {
			...
			newPath.setHeadingInterpolation(closestPoint ->
				MathFunctions.normalizeAngle(Math.PI - path.getHeadingInterpolator().interpolate(closestPoint))
			);
